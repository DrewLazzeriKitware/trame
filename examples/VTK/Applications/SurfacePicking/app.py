...
def update_selection(selectData, **kwargs):
    ...
    tr.update_state("selectData", None)
    tr.update_state("pickingModes", None)
    ...
    with vuetify.VContainer(
        ...
        with vtk.VtkView(
            ...
            with vtk.VtkGeometryRepresentation(
                ...
                color_data_range=("fieldsInformations[field].range",),
            ...
            with vtk.VtkGeometryRepresentation(
                ...
                property=(
                    "{ color: [0.99,0.13,0.37], representation: 0, pointSize: 5 }"
                ),
            ):
                vtk.VtkMesh("selection", state=("selection",))
            with vtk.VtkGeometryRepresentation(
                id="frustrum", actor=("{ visibility: !!frustrum }",)
            ):
...
tr.update_state("f1", vtk_mesh(f1_mesh, point_arrays=["p", "U"]))
...
tr.update_state("fieldsInformations", fieldsInformations)
...
with open("db.html", "w") as out:
    out.write(layout.html)
